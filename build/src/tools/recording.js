...
export const recordStart = defineTool({
    ...
    schema: {
        filePath: z
            .string()
            .optional()
            .describe('The absolute path, or a path relative to the current working directory, to save the recording to. If not provided, a temporary file will be created.'),
        format: z
            .enum(['webm', 'gif'])
            .default('webm')
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            response.appendResponseLine(`Started recording screencast in ${request.params.format} format at ${options.fps} FPS.`);
            if (request.params.filePath) {
