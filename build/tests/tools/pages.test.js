...
import { listPages, newPage, closePage, selectPage, navigatePage, navigatePageHistory, resizePage, handleDialog, } from '../../src/tools/pages.js';
    ...
    describe('browser_navigate_page_history', () => {
        it('go back', async () => {
            await withBrowser(async (response, context) => {
                ...
                await navigatePageHistory.handler({ params: { navigate: 'back' } }, response, context);
        ...
        it('go forward', async () => {
            await withBrowser(async (response, context) => {
                ...
                await page.goto('data:text/html,<div>Hello MCP</div>');
                await page.goBack();
                await navigatePageHistory.handler({ params: { navigate: 'forward' } }, response, context);
                assert.equal(await page.evaluate(() => document.querySelector('div')?.textContent), 'Hello MCP');
                assert.ok(response.includePages);
            });
        });
        it('go forward with error', async () => {
            await withBrowser(async (response, context) => {
                await navigatePageHistory.handler({ params: { navigate: 'forward' } }, response, context);
                assert.equal(response.responseLines.at(0), 'Unable to navigate forward in currently selected page.');
                assert.ok(response.includePages);
            });
        });
        it('go back with error', async () => {
            await withBrowser(async (response, context) => {
                await navigatePageHistory.handler({ params: { navigate: 'back' } }, response, context);
                assert.equal(response.responseLines.at(0), 'Unable to navigate back in currently selected page.');
                assert.ok(response.includePages);
            });
        });
    });
