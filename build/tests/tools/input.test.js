...
import { click, hover, fill, drag, fillForm, uploadFile, } from '../../src/tools/input.js';
...
describe('input', () => {
    ...
    describe('click', () => {
        it('clicks', async () => {
            await withBrowser(async (response, context) => {
                ...
                await page.setContent(`<!DOCTYPE html><button onclick="this.innerText = 'clicked';">test`);
        ...
        it('double clicks', async () => {
            await withBrowser(async (response, context) => {
                const page = context.getSelectedPage();
                await page.setContent(`<!DOCTYPE html><button ondblclick="this.innerText = 'dblclicked';">test`);
                await context.createTextSnapshot();
                await click.handler({
                    params: {
                        uid: '1_1',
    ...
    describe('hover', () => {
        it('hovers', async () => {
            await withBrowser(async (response, context) => {
                const page = context.getSelectedPage();
                await page.setContent(`<!DOCTYPE html><button onmouseover="this.innerText = 'hovered';">test`);
                await context.createTextSnapshot();
                await hover.handler({
                    params: {
                        uid: '1_1',
